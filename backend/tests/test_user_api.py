...
async def test_change_password(client, seed_account, login, portal_db):
    ...
    resp = await client.post(
        ...
        json={"current_password": "testpass", "new_password": "newpass99"},
    )
    assert resp.status_code == 200
    assert b"account set password TESTUSER newpass99 newpass99" in route.calls.last.request.content
...
async def test_2fa_setup_confirm(client, seed_account, login, portal_db, app):
    ...
    # wrong code
    resp = await client.post(
        "/api/v1/user/2fa/confirm", headers=bearer(token), json={"code": "000001"}
    )
    assert resp.status_code == 400 and resp.json()["detail"] == "Invalid code"
    # right code
    route = respx.post(SOAP).mock(return_value=ok("done"))
    good = pyotp.TOTP(body["secret"]).now()
    resp = await client.post("/api/v1/user/2fa/confirm", headers=bearer(token), json={"code": good})
    assert resp.status_code == 200
    assert f"account set 2fa TESTUSER {body['secret']}".encode() in route.calls.last.request.content
...
async def test_2fa_confirm_without_setup(client, seed_account, login):
    ...
    token = await login()
    resp = await client.post(
        "/api/v1/user/2fa/confirm", headers=bearer(token), json={"code": "123456"}
    )
